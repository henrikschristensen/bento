...
func (s *StoreTestSuite) TestMessageStoreSetNextMsgSeqNumRefreshIncrNextMsgSeqNum() {
	...
	s.Require().NoError(s.MsgStore.SetNextSenderMsgSeqNum(867))
	s.Require().NoError(s.MsgStore.SetNextTargetMsgSeqNum(5309))

	// When the store is refreshed from its backing store
	s.Require().NoError(s.MsgStore.Refresh())
	...
	s.Require().NoError(s.MsgStore.IncrNextSenderMsgSeqNum())
	s.Require().NoError(s.MsgStore.IncrNextTargetMsgSeqNum())
...
func (s *StoreTestSuite) TestMessageStoreReset() {
	// Given a MessageStore with the following sender and target seqnums
	s.Require().NoError(s.MsgStore.SetNextSenderMsgSeqNum(1234))
	s.Require().NoError(s.MsgStore.SetNextTargetMsgSeqNum(5678))

	// When the store is reset
	s.Require().NoError(s.MsgStore.Reset())
...
func (s *StoreTestSuite) TestMessageStoreSaveMessageGetMessage() {
	...
	for _, seqNum := range seqNums {
		s.Require().NoError(s.MsgStore.SaveMessage(seqNum, []byte(expectedMsgsBySeqNum[seqNum])))
	}
...
func (s *StoreTestSuite) TestMessageStoreSaveMessageAndIncrementGetMessage() {
	s.Require().NoError(s.MsgStore.SetNextSenderMsgSeqNum(420))
	...
	sort.Ints(seqNums)
	for _, seqNum := range seqNums {
		s.Require().NoError(s.MsgStore.SaveMessageAndIncrNextSenderMsgSeqNum(seqNum, []byte(expectedMsgsBySeqNum[seqNum])))
	}
...
func (s *StoreTestSuite) TestMessageStoreGetMessagesVariousRanges() {
	...
	// Given the following saved messages
	require.NoError(t, s.MsgStore.SaveMessage(1, []byte("hello")))
	require.NoError(t, s.MsgStore.SaveMessage(2, []byte("cruel")))
	require.NoError(t, s.MsgStore.SaveMessage(3, []byte("world")))
