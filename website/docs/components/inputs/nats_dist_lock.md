...
:::
Acquires a distributed lock backed by NATS JetStream KV.
...
input:
  ...
  nats_dist_lock:
    ...
    lock_name: my-singleton-stream # No default (required)
    ttl: 30s
```
...
When used as the first input in a `sequence` input, this component blocks until it acquires a distributed lock and then emits a single message. Subsequent inputs in the sequence only start after the lock is held, ensuring at most one instance of the stream runs across a cluster.

The lock is held and automatically renewed for the lifetime of the stream. When the stream shuts down the lock is released so another instance can take over.
...
<Tabs defaultValue="Singleton stream across a cluster" values={[
{ label: 'Singleton stream across a cluster', value: 'Singleton stream across a cluster', },
]}>

<TabItem value="Singleton stream across a cluster">

Start a stream that only one instance will run at a time. If the leader shuts down, another instance will acquire the lock and take over.
...
The key name of the distributed lock to acquire.
...
The time-to-live for the lock. The lock is automatically renewed at a fraction of this interval to prevent expiry while the stream is running.
...
How long to wait between lock acquisition attempts when the lock is held by another instance.
...
The name of the NATS KV bucket used to store the lock state.


Type: `string`  
Default: `"distributed_locks"`  

### `bucket_replicas`

The number of replicas for the NATS KV bucket. Increase this for clustered NATS deployments.
